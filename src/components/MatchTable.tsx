...
const MatchTable = () => {
    ...
    function dateFormat(timestamp: any) {
        const readableDate = new Date(Number(timestamp)).toLocaleDateString("en-US", {
            year: "numeric",
            month: "long",
            day: "numeric",
        });
    ...
    useEffect(() => {
        ...
        setSeriesDetails(data?.typeMatches
            .find((match: any) => match?.matchType === matchType)
            ?.seriesMatches
            .map((seriesMatch: any) =>
                seriesMatch
            ).find((eachSeries: any) => eachSeries?.seriesAdWrapper?.seriesId == seriesID)?.seriesAdWrapper?.matches)
            ...
            <section className="container mx-auto p-6 font-mono">
                <div className="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
                    <div className="w-full overflow-x-auto">
                        ...
                        <p className="m-3 text-center">You can search your favorite games here 😁</p>
                        <div className="m-6 flex justify-center items-center relative">
                            <div className="relative w-full md:w-1/3">
                                <div className="input-group mb-3">
                                    <div className="input-group-prepend">
                                        <span className="input-group-text" id="basic-addon1" style={{ height: '45px' }}>🔎</span>
                                    </div>
                                    <input
                                        type="text"
                                        className="form-control border-black-300 border-2 rounded-lg py-2 px-4  focus:ring-2 focus:ring-purple-500" style={{ border: 'solid #e9aef2' }}
                                        placeholder="Search..."
                                        aria-label="Search"
                        ...
                        {seriesDetails?.length > 0 ? <>     <table className="w-full table-auto">
                            <thead>
                                <tr className="text-md font-semibold tracking-wide text-left text-white bg-purple-600 uppercase border-b border-gray-200">
                                            ...
                                            <tr className="text-gray-700">
                                                <td className="px-4 py-3 border">
                                                    <div className="flex items-center text-sm">
                                                        ...
                                                        <div>
                                                            <p className="font-semibold text-black">{response?.matchInfo?.seriesName}</p>
                                                            {/* <p className="text-xs text-gray-600">{response?.matchInfo?.matchDesc}</p> */}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3 text-ms font-semibold border">{response?.matchInfo?.status}</td>
                                                <td className="px-4 py-3 text-xs border">
                                                    <span className="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-sm" >
                                                        {response?.matchInfo?.state}
                                                    </span>
                                                </td>
                                                <td className="px-4 py-3 text-sm border">{dateFormat(response?.matchInfo?.seriesStartDt)}</td>
                                            </tr>
                                        </>
                                    })
