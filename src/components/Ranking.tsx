...
const Ranking = () => {
    ...
    const {data} = useGetIccRankingQuery({ playerType: playerType, formatType: formatType });
...
<div className="mt-5 d-flex justify-content-center">
  ...
  >
    ...
    <span
      className="absolute inset-y-0 left-0 m-auto h-6 w-6 rounded-full bg-gray-500 transition-all peer-checked:translate-x-6"
    ></span>
  </label>
...
{data?.rank?.map((player:any)=>{
    ...
    <>
      <tr>
          <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">{player?.rank}</td>
          <td className="whitespace-nowrap px-4 py-2 text-gray-700">{player?.name}</td>
          <td className="whitespace-nowrap px-4 py-2 text-gray-700">{player?.country}</td>
          <td className="whitespace-nowrap px-4 py-2 text-gray-700">{player?.rating}</td>
          <td className="whitespace-nowrap px-4 py-2 text-gray-700">{player?.points}</td>
        </tr>
    </>
    )
})}
